...
{
    $required_fields = ['student_id', 'first_name', 'last_name', 'gender', 'email', 'password', 'password_confirm', 'department', 'year_level', 'section'];
...
{
    $required_fields = ['invitation_code', 'teacher_id', 'first_name', 'last_name', 'gender', 'email', 'password', 'password_confirm'];
    foreach ($required_fields as $field) {
        if (empty($_POST[$field])) {
            echo json_encode(['success' => false, 'message' => "Field '$field' is required."]);
            return;
    ...
    $conn->begin_transaction();
    try {
        // Updated to enforce expiry: require code not used AND (expires_at IS NULL OR expires_at > NOW())
        $stmt_code = $conn->prepare("SELECT id FROM invitation_codes WHERE code = ? AND is_used = FALSE AND (expires_at IS NULL OR expires_at > NOW())");
        $stmt_code->bind_param("s", $_POST['invitation_code']);
        $stmt_code->execute();
        $result_code = $stmt_code->get_result();
        if ($result_code->num_rows === 0) {
            throw new Exception('Invalid, expired, or already used invitation code.');
        }
