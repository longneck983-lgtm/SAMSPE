...
<body>
    ...
    <script>
        ...
        async function loadData() {
            try {
                const res = await fetch('manage_schedule_api.php?action=get_data');
                const json = await res.json();
                if (!json.success) throw new Error(json.message || 'Failed to load data.');
                initialData = json;
                scheduleBody.innerHTML = '';
                if (initialData.schedules.length === 0) {
                    scheduleBody.innerHTML = `<tr><td colspan="${isAdminOrTeacher ? 6 : 5}" style="text-align:center; padding: 2rem;">No schedules have been added yet.</td></tr>`;
                } else {
                    initialData.schedules.forEach(s => {
                        const timeDisplay = `${formatTime(s.start_time)} - ${formatTime(s.end_time)}`;
                        const announcementDisplay = s.announcement ? `<span title="${s.announcement}">${s.announcement.substring(0, 20)}...</span>` : '<span style="color: var(--text-secondary);">None</span>';
                        const actions = isAdminOrTeacher ? `<td><button class="btn btn-secondary" onclick='openScheduleModal(${JSON.stringify(s)})'>Edit</button> <button class="btn" style="background-color:var(--error);color:white;" onclick="deleteSchedule(${s.id})">Delete</button></td>` : '';
                        scheduleBody.innerHTML += `<tr data-department="${s.department}"><td><strong>${s.day_of_week}</strong><br>${timeDisplay}</td><td>${s.course_name}</td><td>${s.section}</td><td>${s.instructor}</td><td>${announcementDisplay}</td>${actions}</tr>`;
                    });
                }

                const deptFilter = document.getElementById('schedule-dept-filter');
                const instructorFilter = document.getElementById('schedule-instructor-filter');
                let deptOptions = '<option value="">All Departments</option>';
                (initialData.departments || []).forEach(d => deptOptions += `<option value="${d.name}">${d.name}</option>`);
                deptFilter.innerHTML = deptOptions;
                let instructorOptions = '<option value="">All Instructors</option>';
                (initialData.teachers || []).forEach(t => {
                    const name = `${t.first_name} ${t.last_name}`;
                    instructorOptions += `<option value="${name}">${name}</option>`;
                });
                instructorFilter.innerHTML = instructorOptions;

                document.getElementById('grace_period_display').textContent = (initialData.rules && initialData.rules.grace_period) ? initialData.rules.grace_period : '--';
                document.getElementById('grace_period_input').value = (initialData.rules && initialData.rules.grace_period) ? initialData.rules.grace_period : '';
            } catch (error) {
                Swal.fire('Error', error.message || 'Could not load data from the server.', 'error');
            }
        }
        
        // --- Redesigned modal with defensive JSON parsing in preConfirm ---
        function openScheduleModal(schedule = null) {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    // Defensive fetch: read text then parse JSON, so parsing errors produce useful message
                    return fetch('manage_schedule_api.php', { method: 'POST', body: formData })
                        .then(async res => {
                            ...
                            try {
                                ...
                                // Include raw server output (trim) for debugging
                                const trimmed = text.length > 1000 ? text.substring(0, 1000) + '...' : text;
                                throw new Error('Invalid server response: ' + (err.message || '') + ' — ' + trimmed);
                            }
                        });
                }
        ...
        function filterSchedules() { const searchTerm = document.getElementById('schedule-search').value.toLowerCase(); const selectedDept = document.getElementById('schedule-dept-filter').value; const selectedInstructor = document.getElementById('schedule-instructor-filter').value; const rows = scheduleBody.querySelectorAll('tr'); let visibleRows = 0; rows.forEach(row => { if (!row.dataset.department) return; const course = row.cells[1].textContent.toLowerCase(); const section = row.cells[2].textContent.toLowerCase(); const instructor = row.cells[3].textContent; const department = row.dataset.department; const matchesSearch = course.includes(searchTerm) || section.includes(searchTerm); const matchesDept = !selectedDept || department === selectedDept; const matchesInstructor = !selectedInstructor || instructor === selectedInstructor; if (matchesSearch && matchesDept && matchesInstructor) { row.style.display = ''; visibleRows++; } else { row.style.display = 'none'; } }); let noResultsRow = scheduleBody.querySelector('.no-results'); if (noResultsRow) noResultsRow.remove(); if (visibleRows === 0 && initialData.schedules && initialData.schedules.length > 0) { scheduleBody.insertAdjacentHTML('beforeend', `<tr class="no-results"><td colspan="${isAdminOrTeacher ? 6 : 5}" style="text-align:center; padding: 2rem;">No schedules match your filters.</td></tr>`); } }
        function toggleRulesEdit(isEditing) { document.getElementById('rules-info-view').style.display = isEditing ? 'none' : 'block'; document.getElementById('rules-edit-view').style.display = isEditing ? 'block' : 'none'; if (document.getElementById('editRulesBtn')) document.getElementById('editRulesBtn').style.display = isEditing ? 'none' : 'inline-flex'; if (document.getElementById('cancelRulesBtn')) document.getElementById('cancelRulesBtn').style.display = isEditing ? 'inline-flex' : 'none'; }
        document.getElementById('rules-edit-view').addEventListener('submit', async function (e) { e.preventDefault(); const formData = new FormData(this); formData.append('action', 'save_rules'); try { const res = await fetch('manage_schedule_api.php', { method: 'POST', body: formData }); const text = await res.text(); if (!text) throw new Error('Empty response from server.'); const data = JSON.parse(text); if (data.success) { await Swal.fire('Success!', data.message, 'success'); await loadData(); toggleRulesEdit(false); } else { throw new Error(data.message || 'Failed to save rules'); } } catch (error) { Swal.fire('Error', error.message || 'Request failed', 'error'); } });

        function deleteSchedule(id) { Swal.fire({ title: 'Are you sure?', icon: 'warning', showCancelButton: true, confirmButtonColor: 'var(--error)', confirmButtonText: 'Yes, delete it' }).then(async (result) => { if (result.isConfirmed) { const formData = new FormData(); formData.append('action', 'delete_schedule'); formData.append('id', id); try { const res = await fetch('manage_schedule_api.php', { method: 'POST', body: formData }); const text = await res.text(); if (!text) throw new Error('Empty response from server.'); const data = JSON.parse(text); if (data.success) { Swal.fire('Deleted!', '', 'success'); loadData(); } else { Swal.fire('Error', data.message || 'Failed to delete', 'error'); } } catch (err) { Swal.fire('Error', err.message || 'Request failed', 'error'); } } }); }
