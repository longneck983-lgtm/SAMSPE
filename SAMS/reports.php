<?php
session_start();
// Allow teacher, admin and superadmin to access reports
    ...
    <script>
        ...
        <?php if ($section_filter) : ?>
            ...
            async function fetchDetailedReport() {
                const url = `reports_api.php?action=get_details&date=<?= $date_filter ?>&section=<?= urlencode($section_filter) ?>`;
            ...
            exportBtn.addEventListener('click', () => {
                const filename = `Attendance_<?= preg_replace('/[^A-Za-z0-9_\-]/', '_', $section_filter) ?>_<?= $date_filter ?>.csv`;
        ...
        <?php else : ?>
            ...
            async function fetchSummaryReport() {
                ...
                const response = await fetch(`reports_api.php?action=get_summary&date=${encodeURIComponent(date)}`);
                const data = await response.json();
                renderSummary(data, date);
            }
            function renderSummary(sections, date) {
                summaryGrid.innerHTML = '';
                if (!Array.isArray(sections) || sections.length === 0) { summaryGrid.innerHTML = '<div class="card" style="grid-column: 1 / -1; text-align: center;">No section data available for this date.</div>'; return; }
                ...
                sections.forEach(sec => {
                    ...
                    const cardHtml = `<a href="?section=${encodeURIComponent(sec.section)}&date=${encodeURIComponent(date)}" class="report-section-card v2"><div class="report-section-header"><h3>${sec.section}</h3><span class="student-count">${sec.total} Students</span></div><ul class="attendance-summary-list"><li class="summary-item"><div class="label-group">${iconPresent} <span>Present</span></div><span class="value present">${sec.present}</span></li><li class="summary-item"><div class="label-group">${iconLate} <span>Late</span></div><span class="value late">${sec.late}</span></li><li class="summary-item"><div class="label-group">${iconAbsent} <span>Absent</span></div><span class="value absent">${absent > 0 ? absent : 0}</span></li></ul></a>`;
